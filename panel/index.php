...
$pages = array (
    ...
    "tag/create"=> array(
        "file_path"=> "tools/tools/tag_creator/create.php",
    ),
    "story/idea"=> array(
        "file_path"=> "tools/tools/story_idea/create.php",
    ),
    "keyword/create"=> array(
        "file_path"=> "tools/tools/key_word/create.php",
    ),
    "post/create_subject"=> array(
        "file_path"=> "tools/tools/subject_creator/create.php",
    ),
    "physic/engineer/list"=> array(
        "file_path"=> "physic/engineer/list.php",
    ),
    "physic/engineer/resistor-color-decode"=> array(
        "file_path"=> "physic/engineer/resistor/decode.php",
    ),
    "physic/engineer/capasitor-number-decode"=> array(
        "file_path"=> "physic/engineer/capasitor/decode.php",
    ),
    "physic/engineer/calc_electronic_data"=> array(
        "file_path"=> "physic/engineer/electronic_data/calc.php",
    ),
    "physic/engineer/calc_coulomb_formula"=> array(
        "file_path"=> "physic/engineer/coulomb/calc.php",
    ),
...
if(!isset($pages[$link]) && !isset($dont_activated[$link]) && explode("/",$link)[0] != "WEB_C"&& explode("/",$link)[0] != "RESULT" && explode("/",$link)[0] != "REMOVE_BACK"&& explode("/",$link)[0] != "QUALITY_UP" && explode("/",$link)[0] != "users"&& explode("/",$link)[0] != "answer_the_question"&& explode("/",$link)[0] != "question_viwe"&& explode("/",$link)[0] != "get_answer"&& explode("/",$link)[0] != "start_chat"){
    ...
    if(isset($e[1]) && isset($e[2])){
        ...
        if($client_data["at_time"]+stay_time > time()){
            ...
            if($operator == "STORY_IDEA"){
                function math_ai_init(){
                    global $client_data;
                    return $client_data;
                }
               require_once(directory."tools/tools/story_idea/handle.php");
            }
            ...
            if($operator == "TAG_CREATOR"){
                function math_ai_init(){
                    global $client_data;
                    return $client_data;
                }
               require_once(directory."tools/tools/tag_creator/handle.php");
            }
            if($operator == "KEYWORD_CREATOR"){
                function math_ai_init(){
                    global $client_data;
                    return $client_data;
                }
               require_once(directory."tools/tools/key_word/handle.php");
            }
            if($operator == "SUBJECT_CREATOR"){
                function math_ai_init(){
                    global $client_data;
                    return $client_data;
                }
               require_once(directory."tools/tools/subject_creator/handle.php");
            }
